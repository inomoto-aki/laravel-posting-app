...
@section('content')
   @if(session('flash_message'))
      <p class="tet-success">{{ session('flash_message') }}</p>
   @endif

   @if(session('error_message'))
     <p class="text-danger">{{ seeion('error_message') }}</p>
   @endif
   ...
   @if($posts->isNotEmpty())
      @foreach($posts as $post)
         <article>
           <div class="card mb-3">
             <div class="card-body">
               <h2 class="crd-title fs-5">{{ $post->title }}</h2>
               <p class="card-title">{{ $post->content }}</p>
               <p>更新日時: {{ $post->updated_at}}</p>

               <div class="d-flex">
                 <a href="{{ route('posts.show', $post) }}" class="btn btn-outline-primarl d-block me-1">詳細</a>
